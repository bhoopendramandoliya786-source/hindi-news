...
export async function GET() {
  ...
  const body = news.map((item) => `<url><loc>${base}/news/${item.slug}</loc><news:news><news:publication><news:name>Hindi News</news:name><news:language>hi</news:language></news:publication><news:publication_date>${(item.publishedAt || new Date()).toISOString()}</news:publication_date><news:title>${escapeXml(item.title.slice(0, 200))}</news:title></news:news></url>`).join("");
