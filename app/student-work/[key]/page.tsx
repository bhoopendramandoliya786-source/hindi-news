...
export default async function StudentWorkPage({ params }: Props) {
  ...
  // Keep this expression deliberately simple for the TSX parser. The previous
  // `as any[]` assertion after the chained flatMap caused Vercel's SWC parser
  // to reject the page before TypeScript could type-check it.
  const matches = categories.flatMap((category) =>
    category.news
      .filter((item) => getStudentEntityKey(item.title, category.slug) === key)
      .map((item) => ({
        ...item,
  ...
  return (
    <main className="container mx-auto max-w-6xl px-4 py-6">
      ...
      <section className="mt-3 rounded-3xl bg-gradient-to-br from-red-700 to-red-500 p-6 text-white shadow-lg sm:p-8">
        ...
        <p className="mt-3 max-w-3xl text-sm leading-7 text-red-50 sm:text-base">यह एक ही भर्ती, परीक्षा, admission, scholarship या student काम के सभी published stages को जोड़ने वाला master page है।</p>
      ...
      <section className="mt-6 rounded-2xl border border-red-100 bg-red-50 p-5">
        ...
        <p className="mt-2 text-sm leading-6 text-gray-700">सबसे नया verified published stage खोलें। अंतिम आवेदन, डाउनलोड, objection, result या status official source पर verify करें।</p>
          ...
          {lifecycle.map((stage) => (
            <div key={stage.label} className={`rounded-2xl border p-4 ${stage.latest ? "border-green-100 bg-green-50" : "border-dashed border-gray-200 bg-gray-50"}`}>
              <div className="flex items-start justify-between gap-2">
                <span className={`flex h-8 w-8 items-center justify-center rounded-full text-sm font-black ${stage.latest ? "bg-green-600 text-white" : "bg-gray-200 text-gray-500"}`}>{stage.index}</span>
                <span className={`rounded-full px-2 py-1 text-[10px] font-black ${stage.latest ? "bg-white text-green-700" : "bg-white text-gray-400"}`}>{stage.latest ? "Update मिला" : "अभी नहीं"}</span>
              </div>
              <p className="mt-3 text-sm font-black text-gray-950">{stage.label}</p>
              {stage.latest ? (
                <>
                  <p className="mt-2 line-clamp-2 text-xs leading-5 text-gray-600">{stage.latest.title}</p>
                  <Link href={`/news/${stage.latest.slug}`} className="mt-3 inline-block rounded-lg bg-gray-950 px-3 py-2 text-xs font-bold text-white">यह stage खोलें →</Link>
                </>
              ) : (
                <p className="mt-2 text-xs leading-5 text-gray-500">इस entity के लिए verified published update अभी नहीं मिला।</p>
              )}
            </div>
          ))}
        </div>
        ...
        <div className="grid gap-5 md:grid-cols-2">
          {matches.slice(0, 30).map((item) => (
            <article key={item.id} className="overflow-hidden rounded-2xl border border-gray-100 bg-white shadow-sm">
              <div className="p-5">
                <div className="flex items-center justify-between gap-3 text-xs"><span className="font-bold text-red-600">{getStudentContext(item.categorySlug, item.title).label}</span><span className="text-gray-400">{new Date(item.publishedAt || item.createdAt).toLocaleDateString("hi-IN", { day: "numeric", month: "short", year: "numeric" })}</span></div>
                <Link href={`/news/${item.slug}`}><h3 className="mt-2 text-lg font-black leading-snug hover:text-red-600">{item.title}</h3></Link>
                <p className="mt-3 line-clamp-3 text-sm leading-6 text-gray-600">{item.description || "Official source पर आधारित student action information."}</p>
                <div className="mt-4 flex items-center justify-between gap-3 border-t border-gray-100 pt-4"><span className="truncate text-xs text-gray-500">Official: {item.sourceName || "Source"}</span><Link href={`/news/${item.slug}`} className="shrink-0 rounded-lg bg-red-600 px-4 py-2 text-xs font-bold text-white">पूरा समझें →</Link></div>
              </div>
            </article>
          ))}
        </div>
      ...
      <section className="mt-7 rounded-2xl border border-gray-200 bg-white p-5">
        ...
        <p className="mt-4 text-xs leading-5 text-gray-500">अंतिम तारीख, fee, vacancy, eligibility और application rules का अंतिम भरोसा official source पर ही रखें।</p>
      </section>
    </main>
  );
}
