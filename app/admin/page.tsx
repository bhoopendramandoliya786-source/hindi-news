...
export default async function AdminPage() {
  ...
  return (
    <main className="min-h-screen bg-gray-50">
      <header className="border-b bg-white"><div className="mx-auto flex max-w-7xl items-center justify-between gap-4 px-4 py-5"><div><h1 className="text-2xl font-black text-gray-950">📰 Hindi News Admin</h1><p className="mt-1 text-sm text-gray-500">Publishing, analytics और website management</p></div><div className="flex flex-wrap gap-2"><a href="/" className="rounded-lg bg-gray-100 px-3 py-2 text-xs font-bold text-gray-700">साइट देखें</a><a href="/admin/login" className="rounded-lg bg-red-600 px-3 py-2 text-xs font-bold text-white">Login</a></div></div></header>
      <section className="mx-auto max-w-7xl px-4 py-8">
        ...
        <div className="mt-8 grid gap-6 lg:grid-cols-[minmax(0,1fr)_320px]">
          <section className="rounded-2xl bg-white p-6 shadow-sm ring-1 ring-gray-200"><div className="flex items-center justify-between gap-3"><div><h3 className="text-xl font-black text-gray-950">🕒 Recent News</h3><p className="mt-1 text-xs text-gray-500">सबसे हाल में बनाई या अपडेट की गई खबरें</p></div><a href="/admin/news" className="text-xs font-black text-red-600">सभी देखें →</a></div><div className="mt-5 divide-y divide-gray-100">{recentNews.map((item) => <a key={item.id} href={`/news/${item.slug}`} className="flex items-center justify-between gap-4 py-4 hover:bg-red-50/50"><div className="min-w-0"><div className="flex flex-wrap gap-2 text-[10px] font-black"><span className="rounded-full bg-gray-100 px-2 py-1 text-gray-600">{item.category.name}</span>{item.status === "DRAFT" && <span className="rounded-full bg-yellow-100 px-2 py-1 text-yellow-800">DRAFT</span>}{item.isOriginal && <span className="rounded-full bg-blue-100 px-2 py-1 text-blue-700">ORIGINAL</span>}{item.isBreaking && <span className="rounded-full bg-red-100 px-2 py-1 text-red-700">BREAKING</span>}</div><h4 className="mt-2 line-clamp-2 text-sm font-bold text-gray-900">{item.title}</h4></div><span className="shrink-0 text-xs font-bold text-gray-400">{item.viewCount.toLocaleString("hi-IN")} views</span></a>)}{!recentNews.length && <p className="py-8 text-center text-sm text-gray-500">अभी कोई खबर नहीं है।</p>}</div></section>
          <aside className="space-y-5"><section className="rounded-2xl bg-white p-6 shadow-sm ring-1 ring-gray-200"><h3 className="font-black text-gray-950">⚡ Quick Actions</h3><div className="mt-4 grid gap-2"><a href="/admin/news" className="rounded-lg bg-red-600 px-4 py-3 text-sm font-bold text-white">➕ नई खबर publish करें</a><a href="/admin/news" className="rounded-lg bg-gray-100 px-4 py-3 text-sm font-bold text-gray-800">📝 Draft manage करें</a><a href="/advertise" className="rounded-lg bg-gray-100 px-4 py-3 text-sm font-bold text-gray-800">💰 Advertising page</a><a href="/contact" className="rounded-lg bg-gray-100 px-4 py-3 text-sm font-bold text-gray-800">📩 Contact / inquiries</a></div></section><section className="rounded-2xl bg-gray-950 p-6 text-white"><h3 className="font-black">📊 Content status</h3><div className="mt-4 space-y-3 text-sm"><div className="flex justify-between"><span className="text-gray-400">Sponsored</span><strong>{sponsored}</strong></div><div className="flex justify-between"><span className="text-gray-400">Breaking</span><strong>{breaking}</strong></div><div className="flex justify-between"><span className="text-gray-400">Categories</span><strong>{categories}</strong></div></div></section></aside>
        </div>
